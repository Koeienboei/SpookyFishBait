...
OBJModel::OBJModel(QString filepathModel, QVector<QString> filepathTextures)
{
    this->filepathModel = filepathModel;
    for (int i=0; i<filepathTextures.size(); i++)
        this->filepathTextures.append(filepathTextures.at(i));
    loadModel();
    initialize();
}
...
OBJModel::~OBJModel() {
    ...
    for (int i=0; i<textureBuffers.size(); i++) {
        textureBuffers.at(i)->destroy();
        free(textureBuffers.at(i));
    }
...
void OBJModel::initialize()
{
    ...
    // Set the number of the used texture
    useTexture = 0;
}

void OBJModel::loadModel()
{
    QFile file(filepathModel);
